...
export default function ResetPasswordScreen({ navigation }) {
  ...
  const verifyEmail = async () => {
    ...
    try {
      ...
      if (!data.exists || !data.actif) {
        Alert.alert('Erreur', 'Ce compte n’existe pas ou n’est pas encore activé.')
  ...
  const changerMotDePasse = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`http://192.168.88.7:8080/clients/changer-motdepasse`, {
        method: 'POST',
      ...
      if (data.success) {
        ...
        Alert.alert('Erreur', data.message || 'Changement échoué')
      }
    } catch (e) {
      Alert.alert('Erreur', 'Une erreur est survenue, veuillez réessayer.')
    }
    ...
    <View style={styles.container}>
      ...
      {emailVerified && (
        <>
          ...
          <View style={styles.buttonContainer}>
            <Button mode="contained" onPress={changerMotDePasse} style={{ marginTop: 24 }}>
              Enregistrer
            </Button>
          </View>
